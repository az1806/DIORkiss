...
<script type="text/javascript">
	function addcpfl() {
		...
		window.location.href = "admin/index?method=addcpfl&cate_name=" + encodeURI(encodeURI(name));
//		$post("index?method=addcpfl", {
	...
	<div class="x-body">
		<div class="layui-row">
			<form class="layui-form layui-col-md12 x-so layui-form-pane">
				 <input class="layui-input" placeholder="分类名" name="cate_name" id="name">
		...
		<span class="x-right" style="line-height:40px"><%=cpfl.size()%>条</span>
		</xblock>
		<table class="layui-table">
			...
			<tbody>
				...
				<tr>
					...
					<td><%=cpfl.get(i).getId()%></td>
					<td><%=cpfl.get(i).getName()%></td>
					<td class="td-manage"><a title="编辑"
	href="admin/index?method=getallcpfl&type=<%=cpfl.get(i).getId()%>">
							<i class="layui-icon">&#xe642;</i> </a> <a title="删除"
						onclick="return confirm('确定要删除么')"
						href="admin/index?method=delcpfl&cpflid=<%=cpfl.get(i).getId()%>"> <i
